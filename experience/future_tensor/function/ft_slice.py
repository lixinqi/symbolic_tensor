...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        async def tracking_get(coords, prompt):
            ...
            return (f"out_{coords}", 1.0)

        ft = FutureTensor([6, 4], tmpdir, tracking_get)
        r = ft_slice(ft, [slice(2, 4), slice(1, 3)])
        run_test("66", r.shape == [2, 2])
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ft = FutureTensor([5, 5], tmpdir, int_get)
        r = ft_slice(ft, [3, slice(None)])
        run_test("75", r.shape == [5])
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ft = FutureTensor([12], tmpdir, step_get)
        r = ft_slice(ft, [slice(0, 12, 4)])
        run_test("78", r.shape == [3])

"""
SleepGradFn: autograd.Function for 2nd-derivative dispatch.
...
class SleepGradFn(torch.autograd.Function):
    """autograd.Function whose forward is a pass-through that creates
    SleepGradFnBackward for 2nd-derivative dispatch."""

    @staticmethod
    def forward(ctx, grad_output):
        from experience.future_tensor.function.sleep_backward import (
            sleep_backward,
        )

        ctx.save_for_backward(grad_output)
        ctx._sleep_backward_fn = sleep_backward
        ctx._grad_input = grad_output

        # 1st-derivative dispatch: policy replaces default backward
        dispatch_1st = get_1st_dispatcher(sleep_backward)
    ...
    def backward(ctx, grad_grad_input):
        ...
        dispatch = get_2nd_dispatcher(ctx._sleep_backward_fn)
        dispatch({

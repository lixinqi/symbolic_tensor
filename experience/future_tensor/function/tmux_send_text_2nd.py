"""
TmuxSendTextGradFn: autograd.Function for 2nd-derivative dispatch.
...
class TmuxSendTextGradFn(torch.autograd.Function):
    """autograd.Function whose forward is a pass-through that creates
    TmuxSendTextGradFnBackward for 2nd-derivative dispatch."""

    @staticmethod
    def forward(ctx, grad_output):
        from experience.future_tensor.function.tmux_send_text_backward import (
            tmux_send_text_backward,
        )

        ctx.save_for_backward(grad_output)
        ctx._tmux_send_text_backward_fn = tmux_send_text_backward
        ctx._grad_input = grad_output

        # 1st-derivative dispatch: policy replaces default backward
        dispatch_1st = get_1st_dispatcher(tmux_send_text_backward)
    ...
    def backward(ctx, grad_grad_input):
        ...
        dispatch = get_2nd_dispatcher(ctx._tmux_send_text_backward_fn)
        dispatch({

...
if __name__ == "__main__":
    ...
    # Test 1: Basic 1D symlink assign
    ...
    # Test 3: Shared storage — writing to source updates lvalue
    ...
    # Test 5: Shape mismatch assertion
    ...
    # Test 6: assign_view with make_none_tensor (touch behavior)
    print("Test 6: Rvalue from make_none_tensor (touch)")
    with tempfile.TemporaryDirectory() as tmpdir:
        src = make_none_tensor([2], tmpdir)  # no storage files
        dst = make_tensor(["a", "b"], tmpdir)
        assign_view(dst, src)
        # Symlinks created, target files touched (empty)
        dst_path = os.path.join(tmpdir, dst.st_tensor_uid, "storage", "0", "data")
        run_test("dst[0] is symlink", os.path.islink(dst_path))
    ...
    # Test 7: assign_view on slice_view result (replace existing symlinks)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Original tensor unchanged (view symlinks now point to new_src)
        run_test("original[0] still 'old_a'", read_storage(original, 0) == "old_a")

    # Test 8: Scalar assign_view
